...
def plot_to_tensorboard(writer, loss_critic, loss_gen,real,fake, tb_step,images=False):
    ...
    if images:
        ...
        with torch.no_grad():
            img_grid_fake = torchvision.utils.make_grid(fake[:4][1], normalize=True) # added [1] so we plot the reconstructed phase not magnitude
            img_grid_real = torchvision.utils.make_grid(real[:4][1], normalize=True)
